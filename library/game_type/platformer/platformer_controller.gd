...
func _physics_process(delta):
	...
	if vel.y > 0: # If we are falling
		emit_signal("on_fall")
		if last_signal != "on_fall":
			emit_signal("on_fall")
			last_signal = "on_fall"
		gravity *= falling_gravity_multiplier
	else:
		if is_on_floor() && last_signal == "on_fall":
			if last_signal != "on_landed":
				emit_signal("on_landed")
				last_signal = "on_landed"
...
func calculate_gravity(_max_jump_height, _jump_duration):
	...
	return (-2 * _max_jump_height) / pow(_jump_duration, 2)


func calculate_jump_velocity(_max_jump_height, _jump_duration):
	# Calculates the desired jump velocity by lookihg at our jump height and jump duration
	return (2 * _max_jump_height) / (_jump_duration)


func calculate_jump_velocity2(_max_jump_height, gravity):
	...
	return sqrt(-2 * gravity * _max_jump_height)


func calculate_release_gravity_multiplier(_jump_velocity, _min_jump_height):
	...
	var release_gravity = 0 - pow(_jump_velocity, 2) / (2 * _min_jump_height)
...
func calculate_speed(_max_speed, _friction):
	# Formula from https://www.reddit.com/r/gamedev/comments/bdbery/comment/ekxw9g4/?utm_source=share&utm_medium=web2x&context=3	
	return (_max_speed / _friction) - max_speed
